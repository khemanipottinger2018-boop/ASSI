...
import {
  ...
  GraduationCap, Shield, Award, DollarSign,
} from 'lucide-react';
...
export default function ProfileCard({
  profile, isPrivate = false, onEdit, onLogout,
}: ProfileCardProps) {
  const initials = profile.username.charAt(0).toUpperCase();

  // Live presence — reads from PresenceProvider already in the tree.
  // Only meaningful when viewing own profile (isPrivate).
  const display  = usePresenceDisplay();
  const isOnline = display.variant === 'available';
    ...
    <div className="max-w-4xl rounded-2xl bg-white/10 backdrop-blur-xl border border-white/10 p-8 space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          {/* Avatar with live presence dot for tutors */}
          <div className="relative">
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-purple-500/30 to-pink-500/20 flex items-center justify-center text-2xl font-semibold text-white">
              {initials}
            </div>
            {isTutor && isPrivate && (
          ...
          <div>
            ...
            {/* Live status label — tutor own profile only */}
            {isTutor && isPrivate && (
              <p className={`text-xs mt-1 ${isOnline ? 'text-emerald-400' : 'text-white/25'}`}>
                {display.label}
              </p>
            )}
          </div>
      ...
      <div className="space-y-3 text-white/80">
        ...
        {profile.tutorBio && (
          <p className="text-white/70 leading-relaxed max-w-2xl">{profile.tutorBio}</p>
        )}
      </div>
      ...
      {/* ── Tutor section ── */}
      {isTutor && (
        <div className="rounded-xl bg-purple-500/10 border border-purple-500/20 p-6 space-y-4">
          <h2 className="text-sm font-semibold text-white/90">Tutor Details</h2>
          ...
          {profile.subjects?.length ? (
            <div className="flex flex-wrap gap-2">
              {profile.subjects.map((s) => (
                <span key={s.id} className="px-3 py-1 rounded-full bg-purple-500/20 text-xs text-white/90">
                  {s.name}{s.category ? ` · ${s.category}` : ''}
                </span>
              ))}
            </div>
          ) : (
            <p className="text-white/40 italic text-sm">No subjects listed</p>
          )}
        </div>
      )}
    </div>
