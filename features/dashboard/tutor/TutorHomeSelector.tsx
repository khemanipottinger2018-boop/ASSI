...
export default function TutorHomeSelector() {
  ...
  const { isConnected, isReady, subscribe } = useSocket();

  // Derive the same booleans the old hook exposed
  ...
  // Delay socket status indicator — avoids a misleading flash on first load
  ...
  const socketLabel = isConnected
    ? variant === 'available' ? 'Discoverable' : 'Connected'
    : 'Socket offline';

  // Incoming session request queue
          ...
          <div className={`panel rounded-2xl p-4 border mb-3 transition-all ${
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex-1">
                {/* Socket status — delayed to avoid flash */}
                {hydrated && showSocketStatus && (
                  <div className="flex items-center gap-1.5 mb-2">
                    <Radio
                      size={9}
                      className={isConnected ? 'text-emerald-400/60' : 'text-white/20'}
                    />
              ...
              {/* Toggle reads from context directly — no props needed */}
