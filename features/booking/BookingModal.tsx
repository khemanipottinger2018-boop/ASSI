...
export interface BookingModalProps {
  ...
  isOnline?:    boolean;
  defaultMode?: BookingMode; // which tab opens first
  ...
  return (
    <div className="flex items-center gap-1.5 mb-6">
      {([1, 2, 3] as Stage[]).map(s => (
        <div key={s} className={`h-0.5 flex-1 rounded-full transition-all duration-400 ${
...
export default function BookingModal({
  ...
  defaultMode = 'scheduled', // NEW — respects caller's intent
  onSuccess,
}: BookingModalProps) {
  ...
  // Reset on open — use defaultMode so caller controls which tab opens
  useEffect(() => {
    if (open) {
      setStage(1);
      setMode(defaultMode);
                  ...
                  <motion.div key="s1"
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <button onClick={() => setMode('instant')} disabled={!isOnline}
                          className={`relative flex flex-col items-start gap-1.5 p-3.5 rounded-2xl border transition-all text-left ${
                            !isOnline ? 'opacity-40 cursor-not-allowed glass-soft border-transparent'
                            : mode === 'instant' ? 'bg-emerald-500/15 border-emerald-500/30 text-emerald-300'
                          ...
                          <div>
                            ...
                            <p className="text-[10px] opacity-60 mt-0.5">{isOnline ? 'Free · 30–60 min' : 'Tutor offline'}</p>
                          </div>
                        </button>
                        <button onClick={() => setMode('scheduled')}
                          className={`flex flex-col items-start gap-1.5 p-3.5 rounded-2xl border transition-all text-left ${
                            mode === 'scheduled' ? 'bg-white/12 border-white/25 text-white'
                            : 'glass-soft border-transparent text-white/50 hover:border-white/15 hover:text-white/70'
                          }`}>
                    ...
                    {error && <p className="text-red-400/80 text-xs flex items-center gap-1.5"><AlertCircle size={11} /> {error}</p>}
                    <button onClick={nextFromStage1}
                  ...
                  <motion.div key="s2"
                    ...
                    <div>
                      ...
                      <input type="datetime-local" value={datetime} min={minDatetime()}
                        ...
                        className="w-full bg-white/6 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white/80 outline-none focus:ring-1 focus:ring-white/15 transition [color-scheme:dark]" />
                    </div>
                    <div>
                        ...
                        {DURATIONS.map(d => (
                          <button key={d} onClick={() => setDuration(d)}
                            className={`flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-medium border transition ${
                              duration === d ? 'bg-white/12 border-white/25 text-white' : 'glass-soft border-transparent text-white/40 hover:text-white/70'
                            }`}>
                            <Clock size={10} /> {formatDuration(d)}
                          </button>
                        ))}
                      </div>
                    </div>
                    <div>
                      ...
                      <textarea value={notes} onChange={e => setNotes(e.target.value)}
                        ...
                        className="w-full bg-white/6 border border-white/10 rounded-xl px-4 py-3 text-sm text-white/70 placeholder-white/20 outline-none focus:ring-1 focus:ring-white/15 transition resize-none" />
                      <p className="text-white/15 text-[10px] text-right mt-1">{notes.length}/500</p>
                    </div>
                    {error && <p className="text-red-400/80 text-xs flex items-center gap-1.5"><AlertCircle size={11} /> {error}</p>}
                    <div className="flex gap-2">
                      <button onClick={() => { setStage(1); setError(''); }}
                        className="flex items-center gap-1 px-4 py-3 rounded-2xl glass-soft text-white/40 text-sm hover:text-white/60 transition">
                        <ChevronLeft size={14} />
                      </button>
                      <button onClick={nextFromStage2}
                  ...
                  <motion.div key="s3"
                    ...
                    <div className="surface rounded-2xl p-4 space-y-3">
                      ...
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="text-white/70 font-medium">{subjects.find(s => s.id === subjectId)?.name ?? '—'}</span>
                      </div>
                      {mode === 'scheduled' && (
                        <>
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="text-white/70 font-medium">
                              {datetime ? new Date(datetime).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' }) : '—'}
                            </span>
                          </div>
                          <div className="flex items-center justify-between text-xs">
                      ...
                      <div className="border-t border-white/8 pt-3">
                        <div className="flex items-center justify-between">
                          ...
                          {currencyLoading ? (
                            ...
                            <span className="text-white/80 text-sm font-semibold">{format(estimatedJMD)}</span>
                          )}
                        </div>
                        {mode === 'scheduled' && (
                          <p className="text-white/20 text-[10px] mt-1">Payment only charged after session completes</p>
                        )}
                      </div>
                    </div>
                    {error && <p className="text-red-400/80 text-xs flex items-center gap-1.5"><AlertCircle size={11} /> {error}</p>}
                    <div className="flex gap-2">
                      <button onClick={() => { setStage(mode === 'instant' ? 1 : 2); setError(''); }} disabled={loading}
                        className="flex items-center gap-1 px-4 py-3 rounded-2xl glass-soft text-white/40 text-sm hover:text-white/60 transition">
                        <ChevronLeft size={14} />
                      </button>
                      <button onClick={handleSubmit} disabled={loading}
                        ...
                        {loading ? <Loader2 size={14} className="animate-spin" />
                          : mode === 'instant' ? <><Zap size={13} /> Start Chat</>
