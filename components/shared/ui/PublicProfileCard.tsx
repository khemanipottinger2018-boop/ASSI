...
// Public-facing tutor profile card.
// Used at /u/[username] — visible to logged-out users too.
// Fully uses the ASSI design system: glass, glass-soft, panel CSS classes + vars.

import { useState } from 'react';
import { useRouter } from 'next/navigation';
import { motion } from 'framer-motion';
import { Star, BookOpen, DollarSign, Clock, MessageCircle, Calendar } from 'lucide-react';
...
export default function PublicProfileCard({ profile }: { profile: UserProfileView }) {
  ...
  // Presence — uses tutor.isAvailable if present (populated by Redis via backend)
  const isOnline = profile.tutor?.isAvailable ?? false;
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="p-6">
          <div className="flex items-start gap-4">
            ...
            <div className="relative flex-shrink-0">
              ...
              {/* Presence dot — only shown for tutors */}
              {isTutor && (
                <span className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-black/30 ${
            ...
            <div className="flex-1 min-w-0">
              ...
              {isTutor && rate != null && (
                <div className="flex items-center gap-3 mt-3 flex-wrap">
                  ...
                  {/* Rating placeholder — not yet from backend */}
      ...
      {/* ── Placeholder for reviews (when backend supports it) ── */}
      {isTutor && (
