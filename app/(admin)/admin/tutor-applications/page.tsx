...
export default function TutorApplicationsPage() {
  ...
  async function loadApplications() {
    try {
      const res  = await fetch(`${API_URL}/api/admin/tutor-applications`, { credentials: 'include' });
  ...
  async function handleAction(id: string, action: 'approve' | 'reject') {
    setActioning(id);
    try {
      await fetch(`${API_URL}/api/admin/tutor-applications/${id}/${action}`, {
        method: 'POST',
        credentials: 'include',
      });
...
function ApplicationCard({
  app, actioning, onApprove, onReject,
}: {
  app: Application;
  actioning: boolean;
  onApprove?: () => void;
